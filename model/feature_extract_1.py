...
'canal_entrada' : {'013': 49, 'KHP': 160, 'KHQ': 157, 'KHR': 161, 'KHS': 162, 'KHK': 10, 'KHL': 0, 'KHM': 12,
                   ...
                   'KCF': 105, 'KCA': 73, 'KCC': 29, 'KCB': 78, 'KCM': 82, 'KCL': 53, 'KCO': 104, 'KCN': 81, 'KCI': 65,
                   ...
                   'KEU': 72, 'KES': 68, 'KEQ': 138, 'nan': 6, 'KFV': 48, 'KFT': 92, 'KFU': 36, 'KFR': 144, 'KFS': 38,
...
def add_com_features(lag_feats):
    ...
    for gp_col in ['renta', 'pais_residencia','canal_entrada']:
...
if __name__ == "__main__":
    ...
    train_file = train_file[train_file['fecha_dato'].isin(data_date)].loc[:,['ncodpers', 'fecha_dato'] + target_raw_cols]
    ...
    df_train.to_csv(data_path + 'feats/train_feats_v2.csv', index=False)
    df_test.to_csv(data_path + 'feats/test_feats_v1.csv', index=False)
    print datetime.datetime.now() - start_time